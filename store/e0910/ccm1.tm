...
<\body>
  ...
  est la completion par la norme <math|\<bbb-E\>[\|\<cdot\>\|<rsup|2>]> de
  l'ensemble des fonction étagées. Les elements de
  <math|L<rsup|2>(\<cal-F\>)> sont les classes d'equivalence des fonctions
  mesurables selon la relation <math|X\<sim\>Y> ssi <math|\<bbb-P\>(X=Y)=1>.
  ...
  <\proof>
    ...
    un autre projection orthogonale. On a que
  ...
  <\proof>
    ...
    <math|L<rsup|1>>. Soit <math|X<rsub|n>(\<omega\>)=max(X(\<omega\>),n)> et
