...
<\body>
  ...
  <math|u<rsub|f>(x)> pour tout <math|x\<in\>B> où <math|B> est n'importe
  quel evenement tel que <math|\<bbb-P\>(X\<in\>B)=0> sans affecter la
  propriété<eqref|eq:equi>. L'avantage majeure de cette nouvelle définition
  \ est sa généralité.
  ...
  <\example>
    ...
    <\equation*>
      <with|math-font|cal|\<sigma\>>(X)=<left|{>\<emptyset\>,\<Omega\>,X<rsup|-1>({0}),X<rsup|-1>({1})<right|}>.
    </equation*>
  </example>
  ...
  <\proof>
    ...
    <\enumerate>
      ...
      (vérifier)<math|>. Par symétrie on a aussi que
  ...
  <\example>
    ...
    Il suffit de vérifier que \ la constante <math|\<bbb-E\>[X]> satisfait
    bien la définition (<reference|eq:def-cond-exp>).
  </example>
  ...
  <\exercise>
    ...
    selon la définition usuelle d'espérance conditionnelle de <math|g(Y)>
    sachant <math|X=x> pour les vecteurs aléatoires avec densité.
  </exercise>
