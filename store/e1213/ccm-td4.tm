...
<\body>
  ...
  <\exercise>
    ...
    <math|\<cal-M\>=\<bbb-R\>\<cup\><around|{|\<Delta\>|}>>: à un instant
    ...
    <math|\<cal-A\>=<around|{|0,1|}>>, <math|0> si on n'exerce pas et
    <math|1> si on décide d'exercer l'option.
    ...
    <\enumerate-alpha>
      ...
      rapport à la suite iid <math|<around|(|\<varepsilon\><rsub|n>|)><rsub|n\<geqslant\>1>>.
      ...
      et <math|V<around|(|N,x|)>=0> si <math|x\<in\>\<bbb-R\>>.

      <item>Par récurrence rétrograde montrer que <math|V<around|(|k,x|)>>
      est une fonction convexe de <math|x> pour tout
      <math|0\<leqslant\>k\<leqslant\>N>.

      <item>Par récurrence rétrograde montrer que
      <math|V<around|(|k,x|)>\<geqslant\>V<around|(|k+1,x|)>> pour tout
      <math|0\<leqslant\>k\<leqslant\>N-1>.

      <item>Soit <math|p<rsub|k>=inf<around|{|x\<geqslant\>0 :
  ...
  <\exercise>
    ...
    probabilité <math|p\<in\><around|]|0,1|[>> que la queue avance d'une
    position. On estime que l'utilité d'arriver à être servi soit
    <math|r\<gtr\>0> et que attendre dans la queue nous coût <math|c\<gtr\>0>
    à chaque instant du temps. On a toujours la possibilité de partir da la
    queue et de ne pas être servit. On veut trouver une stratégie (a savoir
    si on doit rester dans la queue et attendre ou partir) qui maximise notre
    gain moyen.

    On modélisera le problème avec un système dynamique contrôlé homogène sur
    l'espace d'états <math|\<cal-M\>=\<bbb-N\>\<cup\><around|{|\<Delta\>|}>>:
    à un instant déterminé soit on est dans la queue et on a
    <math|x\<in\>\<bbb-N\>> clients avant nous, soit on est parti et alors on
    décide de façon conventionnelle d'être dans l'état fictif
    <math|\<Delta\>>. L'espace des actions est
    <math|\<cal-A\>=<around|{|0,1|}>>, <math|0> si on reste dans la queue
    pour un autre interval de temps et <math|1> si on décide de partir.
    ...
    <\enumerate-alpha>
      ...
      <\equation*>
        V<around|(|x|)>=<around|(|V<around|(|x-1|)>-c/p|)><rsub|+>,<space|2em>x\<geqslant\>1.
      </equation*>

      <item>Montrer par récurrence que <math|V<around|(|x|)>\<leqslant\>V<around|(|x-1|)>>.
      Donner une explication intuitive de cette inégalité.\ 
      ...
      <item>Déterminer <math|V<around|(|x|)>> et <math|x<rsup|\<ast\>>> en
      fonction de <math|r,c,p>.
    </enumerate-alpha>
  </exercise>
